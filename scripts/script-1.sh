#!/bin/bash
# Stress Test Script - Group 1 (Light Load - 25% CPU)
# This script sets up the VM and prepares it for stress testing

set -e

echo "=========================================="
echo "Stress Test Group 1 - Light Load Setup"
echo "=========================================="
...
# Stress Test Configuration - Group 1
STRESS_LEVEL="light"
CPU_PERCENT=25
MEMORY_PERCENT=30
IO_WORKERS=1
TIMEOUT=0  # 0 = infinite, set to seconds for finite test
EOF
...
CPU_WORKERS=${CPU_WORKERS:-1}  # At least 1 worker
...
echo "Starting Stress Test - Group 1 (Light Load)"
echo "==========================================="
...
# Create systemd service for automatic stress testing
cat > /etc/systemd/system/stress-test.service << 'EOF'
[Unit]
Description=Stress Test Service - Group 1
...
# Don't start the service automatically - wait for manual trigger
...
echo "✅ Stress Test Group 1 setup complete!"
